...
{
    public partial class SuReconcile : System.Web.UI.Page
    {
...
LEFT JOIN SUPPLIERINFO_MST B ON A.SUID=B.SUID";
            ...
            sqb.AppendFormat(" WHERE B.SNAME LIKE  '%" + Text1 .Value  + "%'");
            if (CheckBox1.Checked)
            {
                sqb.AppendFormat(" AND CONVERT(VARCHAR(10),A.Date,112)>= '" + StartDate.Value + "'  AND CONVERT(VARCHAR(10),A.Date,112)<='" + EndDate.Value + "'");
            }  
            sqb.AppendFormat(" AND A.SNID LIKE '%{0}%'", Text2.Value );
            sqb.AppendFormat("  ORDER BY  A.SNID ASC");
        ...
        protected void nextpage()
        {
            ...
            lblSurrentIndex.Text = "当前页第" + ((GridView1.PageIndex) + 1).ToString() + "页";
            if (dt.Rows.Count > 0)
            {
        ...
        protected void GridView1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            WPSS.PurchaseManage.SuReconcileT.NEWID[0] = str[0];
            string n1 = Request.Url.AbsoluteUri;
            string n2 = n1.Substring(n1.Length - 16, 16);
            Response.Redirect("../PurchaseManage/Sureconcilet.aspx"+n2);
        ...
        protected void GridView1_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            sqb.AppendFormat("DELETE FROM Su_RECONCILE_MST WHERE SNID='" + ID + "'");
            sqb.AppendFormat(";DELETE FROM Su_RECONCILE_DET WHERE SNID='" + ID + "'");
            string s1 = bc.getOnlyString("SELECT STATUS FROM [Su_RECONCILE_DET] WHERE SNID='" + ID + "'");
            if (s1 == "RECONCILE")
            {
                hint.Value = "此供应商对账单已对账，不允许删除";
            }
            ...
            {
                dt = cpurchase_reconcile.RETURN_PG_AND_RETURN_DT2(ID , "", "", "", "", "", "", false, true);
                DataTable dtx = bc.RETURN_NOHAVE_REPEAT_DT(dt, "入库退货单号");
                foreach (DataRow dr in dtx.Rows)
                {
                    if (dr["VALUE"].ToString().Substring(0, 2) == "PG")
                    {
                        sqb.AppendFormat(";UPDATE PurchaseGode_DET SET STATUS='N' WHERE PGID='" + dr["VALUE"].ToString() + "'");
                    }
                    else
                    {
                        sqb.AppendFormat(";UPDATE RETURN_DET SET STATUS='N' WHERE REID='" + dr["VALUE"].ToString() + "'");
                    }
                }
        ...
        {
            hint.Value = "";
            string var2 = new CPURCHASE_RECONCILE().GETID();
            SuReconcileT.NEWID[0] = var2;
            string n1 = Request.Url.AbsoluteUri;
            string n2 = n1.Substring(n1.Length - 16, 16);
            Response.Redirect("../PurchaseManage/Sureconcilet.aspx"+n2);
        }
